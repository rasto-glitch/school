...
- **ClamAV deferred to Wave 3.** Current upload defenses are MIME allowlist + magic-byte sniff + `Content-Disposition: attachment` on the stored object + 10 MB cap + private bucket + signed-URL-only reads + per-doc SHA-256 (chain of custody). This neutralizes the H-2 class of attacks (HTML/SVG inline render, mismatched Content-Type). What it doesn't catch: macro-laden Office files, embedded payloads in PDFs, exploit chains that target the viewing user's machine after download. If a school's staff laptop fleet becomes a target vector, wire a ClamAV worker to flip `employee_documents.scan_status` from `skipped`/`pending` to `clean`/`infected`; the signed-URL endpoint already blocks `infected` reads. Until then, `scan_status` ships as `skipped` and the field exists so the upgrade path is one Railway service + one cron.
