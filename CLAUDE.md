...
- **Subjects are denormalized.** Both `teachers.subject` (text) and a `subjects` table exist. Teacher profile endpoint prefers the `subjects` row and falls back to `teachers.subject`. Don't assume a single source.
