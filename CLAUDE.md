...
- **Students are archived, not soft-deleted.** `archiveStudent()` copies the student into `archived_students` (snapshotting `enrollment_history` and `grades` as JSONB), then deletes the original row. `archived_students` is not cascaded from `students`. Per-year academic progression lives in the `student_enrollments` table for live students; on archive it serialises into `archived_students.enrollment_history`. The legacy `classes_attended` JSONB column is kept (always written as `[]` for new archives) because migration 019's tamper-evidence hash `_canon_archived_student` includes it in its canonical input — dropping it would break integrity verification on every existing archive. See migration 030.
- **Hard student deletes cascade everything.** When a student is hard-deleted (`deleteStudent`, or graduation/year-transition on an archive-OFF school), `attendance`, `bus_ride_records`, `student_enrollments`, `grades` (cascade), and `reports` (cascade) all go away with the student row. There is no per-table snapshot. Archive-off schools accept this as the cost of the cheaper data lifecycle. For archive-ON schools, the year-end frozen `attendance_totals` on `student_enrollments` survives the cascade because it's captured into `archived_students.enrollment_history` BEFORE the cascade fires — `closeCurrentEnrollment` is called before the snapshot/delete. Migration 041 added the same survive-the-cascade story for `reports` via `archived_students.reports` JSONB.
- **Authoritative academic year = `schools.current_academic_year`.** Migration 042 made this column the source of truth. Resolve it via `backend/src/utils/studentEnrollments.ts#resolveCurrentAcademicYear(schoolId)` — it consults the column and falls back to the September boundary (`academicYearOf()`) only when the column is NULL or doesn't match `\d{4}-\d{4}`. The year-transition wizard, School Settings → academic year, and the per-class promote wizard all write here. Every write path that stamps `academic_year` onto a new row (createReport, upsertGrade, openEnrollmentForCurrentYear, updateClassForCurrentYear, closeCurrentEnrollment, markOnLeaveForCurrentYear, returnFromLeave) calls the resolver. **Do not** reach for `academicYearOf()` in a new writer — it ignores the school's override.
...
- Stamping `academic_year` on a new row → call `resolveCurrentAcademicYear(schoolId)`, not `academicYearOf()`.
