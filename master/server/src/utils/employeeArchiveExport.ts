// PDF + Excel export of a school's archived employees. Mirrors
// backend/src/utils/employeeArchiveExport.ts but uses the master server's
// own Supabase client (passed in). Kept duplicated rather than shared
// because master is a local-only workspace with no shared package boundary
// to backend/ (same rationale as archiveExport.ts).
...
export async function loadEmployeeArchiveSnapshot(supabase: SupabaseClient, schoolId: string): Promise<EmployeeArchiveSnapshot> {
