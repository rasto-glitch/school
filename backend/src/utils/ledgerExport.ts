...
export async function streamLedgerPdf(stream: Writable, data: LedgerExportData): Promise<void> {
  ...
  if (data.categories.length > 0) {
    ...
    for (const [currency, list] of byCurrency) {
      ...
      for (const c of income) {
        ...
        doc.font(F.pick(c.category)).fontSize(10).fillColor(COLOR_HEADING).text(c.category, leftX, yL, { width: labelW, ellipsis: true, lineBreak: false });
      ...
      for (const c of expense) {
        ...
        doc.font(F.pick(c.category)).fontSize(10).fillColor(COLOR_HEADING).text(c.category, rightX, yR, { width: labelW, ellipsis: true, lineBreak: false });
  ...
  doc.font(F.pick(data.schoolName)).fontSize(9).fillColor(COLOR_MUTED).text(
    ...
    M, doc.page.height - 65, { width: CONTENT_W, align: 'center', lineBreak: false },
  );
  doc.end();
}
