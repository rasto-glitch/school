...
export async function streamStaffSalaryPdf(stream: Writable, data: StaffSalaryExportData): Promise<void> {
  ...
  doc.font(F.pick(data.fullName,{bold:true})).fontSize(13).fillColor(COLOR_HEADING).text(data.fullName, 40, 142);

  let metaY = 168;
  if (data.position) {
    doc.font(F.regular).fontSize(9).fillColor(COLOR_MUTED).text('Position', 40, metaY);
    doc.font(F.pick(data.position,{bold:true})).fontSize(10).fillColor(COLOR_HEADING).text(data.position, 110, metaY);
    metaY += 16;
  }
