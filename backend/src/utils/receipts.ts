...
function drawHeader(doc: PDFKit.PDFDocument, school: SchoolInfo, logoBuf: Buffer | null, subtitle: string) {
  ...
  doc.font(F.pick(school.name,{bold:true})).fontSize(18).fillColor(COLOR_HEADING).text(school.name, 110, 48);
...
function field(doc: PDFKit.PDFDocument, label: string, value: string, x: number, y: number, width: number) {
  ...
  doc.font(F.pick(value,{bold:true})).fontSize(11).fillColor(COLOR_HEADING).text(value, x, y + 12, { width });
}
...
export async function streamPaymentReceipt(stream: Writable, data: PaymentReceiptData): Promise<void> {
  ...
  if (data.unallocatedAmount > 0) {
    ...
    if (data.unallocatedNote) {
      doc.font(F.pick(data.unallocatedNote)).fontSize(10).fillColor(COLOR_HEADING).text(data.unallocatedNote, 40, extraY + 12, { width: 515 });
  ...
  if (data.notes) {
    ...
    doc.font(F.pick(data.notes)).fontSize(10).fillColor(COLOR_HEADING).text(data.notes, 40, extraY + 12, { width: 515 });
  ...
  // Footer
  doc.font(F.pick(data.school.name)).fontSize(9).fillColor(COLOR_MUTED).text(
