...
export async function getOrCreateConversation(req: AuthRequest, res: Response): Promise<void> {
  ...
  if (role === 'parent') {
    ...
    const { data: other } = await supabase.from('users').select('role').eq('id', otherUserId).single();
    ...
    const { data: other } = await supabase.from('users').select('role, school_id').eq('id', otherUserId).single();
