...
import { User, Plus, Pencil, Building2 } from 'lucide-react-native';
...
// Profile/Me tab for EmployeeTabs (admin / accountant / staff) — roles whose
// only mobile purpose is clocking in. Mirrors ReceptionMeScreen, with a role
// label resolved from the signed-in user's role.
export default function EmployeeMeScreen() {
  ...
  const roleLabel = user?.role ? t(`nav.${user.role}`, user.role) : '';
